...
def _download_url_file(url: str, dst_path: str) -> str:
    ...
    try:
        ...
        raise ModelError(
            f"Failed to download SatMAE++ source file from {url}: {e}"
        ) from e
    with open(dst_path, "wb") as f:
        f.write(payload)
    if not (os.path.exists(dst_path) and os.path.getsize(dst_path) > 0):
        raise ModelError(f"Downloaded SatMAE++ source file is empty: {dst_path}")
    return dst_path
...
def _ensure_satmaepp_s2_assets(
    *,
    ...
    root = cache_dir or os.path.join(os.path.expanduser("~"), ".cache")
    code_root = os.path.join(root, "rs_embed", "satmaepp_s2_source")
    util_dir = os.path.join(code_root, "util")
    os.makedirs(util_dir, exist_ok=True)

    # Keep util import robust on older Python import machinery.
    util_init = os.path.join(util_dir, "__init__.py")
    if not os.path.exists(util_init):
        with open(util_init, "w", encoding="utf-8") as f:
            f.write("# auto-generated by rs-embed for satmaepp_s2 runtime import\n")

    base = f"https://raw.githubusercontent.com/{code_repo}/{code_ref}"
    model_py = _download_url_file(
        f"{base}/models_mae_group_channels.py",
        os.path.join(code_root, "models_mae_group_channels.py"),
    )
    _download_url_file(
        f"{base}/util/pos_embed.py",
        os.path.join(util_dir, "pos_embed.py"),
    )
    return model_py, ckpt_path, code_root


def _load_satmaepp_s2_module(model_py: str, code_root: str):
    if not (os.path.exists(model_py) and os.path.isfile(model_py)):
        raise ModelError(f"SatMAE++ source file missing: {model_py}")

    digest = hashlib.sha1(model_py.encode("utf-8")).hexdigest()[:10]
    module_name = f"satmaepp_s2_impl_{digest}"
    ...
    try:
        ...
        if spec is None or spec.loader is None:
            raise ModelError(
                f"Failed to create import spec for SatMAE++ source: {model_py}"
            )
        mod = importlib.util.module_from_spec(spec)
        spec.loader.exec_module(mod)
    except Exception as e:
        raise ModelError(
            f"Failed to import SatMAE++ source module: {type(e).__name__}: {e}"
        ) from e
    finally:
        sys.path = prev_path
...
def _load_satmaepp_s2_cached(
    *,
    ...
    model_py, ckpt_path, code_root = _ensure_satmaepp_s2_assets(
        ckpt_repo=ckpt_repo,
        ckpt_file=ckpt_file,
        code_repo=code_repo,
        code_ref=code_ref,
    ...
    mod = _load_satmaepp_s2_module(model_py, code_root)
...
class SatMAEPPSentinel10Embedder(EmbedderBase):
    ...
    DEFAULT_CODE_REPO = "techmn/satmae_pp"
    ...
    def describe(self) -> Dict[str, Any]:
        return {
            ...
            "defaults": {
                ...
                "code_repo": self.DEFAULT_CODE_REPO,
                "code_ref": self.DEFAULT_CODE_REF,
    ...
    def get_embedding(
        ...
        *,
        ...
        code_repo = os.environ.get(
            "RS_EMBED_SATMAEPP_S2_CODE_REPO", self.DEFAULT_CODE_REPO
        ).strip()
        code_ref = os.environ.get(
            "RS_EMBED_SATMAEPP_S2_CODE_REF", self.DEFAULT_CODE_REF
        ).strip()
