...
| `anysat` | AnySat from upstream `hubconf.py` (`AnySat`, `tiny` / `small` / `base`) | S2 10-band TCHW (or CHW auto-expanded) | clip to `0..10000`; normalize mode default `per_tile_zscore`; builds per-frame `s2_dates` | resize TCHW to default 24; no crop, no pad | patch output `[D,H,W]`, pooled by spatial mean/max | Medium |
...
| `prithvi` | Vendored `PrithviMAE` runtime with HF checkpoints | S2 6-band (`BLUE,GREEN,RED,NIR_NARROW,SWIR_1,SWIR_2`) | raw SR -> `/10000` -> clamp `[0,1]`; prep mode from env | default mode `resize` to 224; optional `pad` to patch multiple (legacy) | token sequence -> pooled or patch-token grid | Medium |
...
| `dofa` | DOFA ViT (`base` / `large`, official checkpoints) | multi-band SR CHW + wavelengths | raw SR -> `/10000` to `[0,1]`; provide/infer wavelengths | bilinear resize to 224; explicitly no crop/pad | pooled vector or token grid (usually 14x14) | Medium-High |
...
| `thor` | Fully vendored THOR runtime (`tiny` / `small` / `base` / `large`) | S2 SR 10-band | clip `0..10000`; default `thor_stats` z-score after reflectance scaling | default 288; bilinear resize; no pad | pooled tokens and grouped token grid | Medium-High |
