...
| `thor`            | Fully vendored THOR runtime (`tiny` / `small` / `base` / `large`)                                          | 10m                      | S2 SR 10-band or S1 VV/VH                             | S2: clip `0..10000`, then default `thor_stats` z-score after reflectance scaling; S1: shared `log1p` + p99 scaling by default                           | default 288; bilinear resize; no pad                                    | pooled tokens and grouped token grid                                                                                   | Medium-High                                                                            |
